"""Operational Excellence pillar agent implementation."""
...
from .operational_constants import DOMAIN_TITLES, PILLAR_CODE, PILLAR_PREFIX


class OperationalAgent(BasePillarAgent):
    """Agent specialized in Azure Well-Architected Operational Excellence pillar assessment."""

    def __init__(self):
        """Initialize the Operational Excellence agent."""
        super().__init__(
            pillar_code=PILLAR_CODE,
            pillar_prefix=PILLAR_PREFIX,
            domain_titles=DOMAIN_TITLES,
            instructions_filename="operational_agent_instructions.txt",
            agent_name="OperationalExcellenceAgent",
            pillar_display_name="Operational Excellence",
            mcp_topic="operational-excellence",
        )


async def main():
    """CLI entry point for Operational Excellence agent."""
    import sys
    from pathlib import Path
    from backend.app.utils.cli_utils import create_agent_argument_parser, apply_log_level_override

    parser = create_agent_argument_parser("Operational Excellence")
    ...
    init_logging()
    print("Initializing Operational Excellence Agent...")
    ...
    try:
        ...
        artifacts = agent.write_assessment_artifacts(
            assessment,
            markdown_filename="operational_excellence_assessment.md",
            json_filename="operational_excellence_assessment.json",
        )
        print("\nResults written to:")
