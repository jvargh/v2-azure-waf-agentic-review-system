...
from backend.app.tools.mcp_tools import MCPToolManager
from backend.utils.env_utils import EnvironmentConfig, load_env_vars, validate_env_vars
from backend.utils.scoring.scoring import compute_pillar_scores, summarize_scores
...
class BasePillarAgent:
    ...
    def __init__(
        ...
        *,
        ...
        self.prompts_dir = self.project_root / "src" / "app" / "prompts"
    ...
    def _parse_response(
        ...
        # --- Evidence floor (parity with server conservative scoring) ---
        # If the LLM returned an overall score of 0 but there is ANY evidence present
        # (non-zero domain scores or sufficiently large architecture description),
        # apply a small visibility floor so downstream UI/components don't show
        # an opaque zero which users interpret as a processing failure.
        if overall_score == 0:
            total_domains = len(domain_scores)
            non_zero_domains = sum(1 for v in domain_scores.values() if isinstance(v, dict) and v.get("score", 0) > 0)
            corpus_size = len(architecture_content or "")
            evidence_indicator = non_zero_domains > 0 or corpus_size > 800
            if evidence_indicator and total_domains > 0:
                # Coverage proxy = % of domains with some score
                coverage_pct = (non_zero_domains / total_domains) * 100
                coverage_floor = int(max(12, min(25, coverage_pct * 0.3)))  # mirrors server logic
                logger.info(
                    "[evidence-floor] Applied floor to overall_score (raw=0, domains_with_evidence=%d/%d, coverage=%.1f%%, corpus_size=%d) -> %d",
                    non_zero_domains,
                    total_domains,
                    coverage_pct,
                    corpus_size,
                ...
                logger.info(
                    "[evidence-floor] True zero evidence detected (domains_with_evidence=%d/%d, corpus_size=%d) -> score remains 0",
                    non_zero_domains,
                    total_domains,
                    corpus_size,
                )

        maturity_scores = compute_pillar_scores(architecture_content, pillar=self.pillar_code)
        maturity_summary = summarize_scores(maturity_scores)
        ...
        return PillarAssessment(
            ...
            architecture_text=architecture_content,
        )
